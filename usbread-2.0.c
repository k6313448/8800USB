...
int form_json_list(usb_buf_send** inputList) {
	usb_buf_send *rec, *pre = NULL;
	int cnt;
	struct json_object *tempdata = NULL, *array;
	array = json_object_new_array();
	if (NULL == array) {
		printf("array: json_object_new_array err\n");
		return -1;
	}

	for (rec = *inputList, cnt = 0; rec != NULL && cnt < totalcnt_g; rec = rec->next, cnt++) {
		tempdata = json_object_new_object();
		if(NULL == tempdata) {
			printf("tempdata: json_object_new_object err\n");
			json_object_put(array);
			return -1;
		}
		json_data_creat(tempdata, rec);
		json_object_array_add(array, tempdata);
		if(*inputList != NULL) {// remove head
	...
	sprintf(data_json, "list=%s", (char *)json_object_to_json_string(array));
	json_object_put(array);

	return 0;
}

int SendJsonData(usb_buf_send ** list){
	form_json_list(list);
	printf("data_json: %s\n", data_json);
...
void* thread_send_json(usb_buf_send ** list){
...
int main(int argc, char **argv) {
	...
	totalcnt_g = (cachesize_g)/128;
	interval_g = atoi_dec(argv[6]);
	data_json = (char*)malloc(cachesize_g); 
	...
	printf("ip: %s\nport: %s\npath: %s\ncacheSize: %dB\ninterval: %ds\n", IP, PORT, PATH, cachesize_g, interval_g);

	pthread_t ntid;
	pthread_create(&ntid, NULL, thread_send_json, &list_data);
